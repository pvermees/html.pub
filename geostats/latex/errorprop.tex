...
significantly greater than 1 Myr, then such a causal relationship
remains plausible. There are two aspects of analytical uncertainty
...
\noindent\includegraphics[]{../figures/accuracyvsprecision.pdf}
...
Suppose that a physical quantity ($z$) is calculated as a function
($g$) of some measurements ($x$):
\begin{equation}
z = g(x)
\label{eq:zgx}
\end{equation}
...
from the mean value $\bar{x}$ are proportional to small deviations
...
\noindent\begin{minipage}[t][][b]{.45\textwidth}
  \includegraphics[]{../figures/errorprop2d.pdf}\medskip
\end{minipage}
...
\noindent where the innermost matrix is known as the
\emph{variance-covariance} matrix and the outermost matrix (and its
transpose) as the \emph{Jacobian matrix}. The advantage of the matrix
formulation is that it can easily be scaled up to three or more
dimensions.

\noindent\begin{minipage}[t][][b]{.55\textwidth}
  \includegraphics[]{../figures/errorprop3d.pdf}\medskip
\end{minipage}
...
\item{$a, b, c, \ldots$} are constants, i.e. values that are known
  without uncertainty ($s[a]=s[b]=s[c]=\ldots=0$);
...
\item{\bf other:}
  ...
  function of time $t$, where $d_0$ is the position at $t=0$,
  $v_0$ is the velocity at $t=0$, and $g$ is the acceleration.
  ...
  \[
  s[d]^2 = \left(v_0 s[t]\right)^2 + \left(2 g t s[t]\right)^2
  \]

  \noindent which leads to the following expression for the
  uncertainty of $d$:
  \begin{equation}
    s[d] = \sqrt{\left(v_0 s[t]\right)^2 + \left(2 g t s[t]\right)^2}
    = s[t] \sqrt{v_0^2 + \left(2 g t\right)^2}
    \label{eq:sdnewton}
  \end{equation}

  To illustrate the use of this formula, suppose that $d_0=0$~m,
  $v_0=10$~m/s and $g=9.81$~m/s\textsuperscript{2}.  Further
...
Thus we have proved that the variance of a Poisson variable equals its
