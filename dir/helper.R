...
# returns the coordinates of a Sierpinski carpet
sierpinski <- function(){
    f <- function(x) {
        z <- x
        z[] <- ' '
...
randy <- function(pop=1,n=250){
    if (pop == 1){
        ...
        rx[fromhead1] <-  M
        ry[fromhead1] <-  0.5+0.25*(rnum[fromhead1]-cutoff1)/(cutoff2-cutoff1)
        rx[fromhead2] <-  0.5+0.25*(rnum[fromhead2]-cutoff2)/(1-cutoff2)
        ry[fromhead2] <-  M
    } else if (pop == 3){
        x <- c(0.25,0.75,0.75,0.25,0.25)
        y <- c(0.25,0.25,0.75,0.75,0.25)
...
plot.circ <- function(angles,...){
    ...
    text(-1-tl,0,labels='360',pos=2)
}

points.circ <- function(angles,...){
    rads <- angles*pi/180
